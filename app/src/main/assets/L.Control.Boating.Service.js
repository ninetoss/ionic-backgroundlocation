L.Control.Boating = L.Control.extend({
    options: {
        ...
        circleColor: '#3388ff',
        cacheLength: 4,
        url: 'location.json', // Legacy internal polling (optional now)
        interval: 5000
    ...
    stop: function () {
        ...
        this.stopPolling(); // Stop JS polling if active
    },
    ...
    fetchLocation: function () {
        fetch(this.options.url + '?t=' + Date.now())
            .then(response => response.json())
            .then(data => {
                const eventData = {
                    latlng: L.latLng(parseFloat(data.lat), parseFloat(data.lng)),
                    heading: parseFloat(data.bearing),
                    speed: parseFloat(data.speed),
                    timestamp: parseFloat(data.timestamp),
                    accuracy: 10
                }
                this.onLocationFound(eventData)
            })
            .catch(err => {
    ...
    onLocationFound: function (e) {
        ...
        if (this.isRequesting()) {
            this.sessionData = {
                startTime: new Date(),
                totalDistance: 0,
                path: [e.latlng], // Initialize path array
                lastLatLng: e.latlng
            };
